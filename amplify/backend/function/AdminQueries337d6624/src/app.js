...
app.get("/lawyerApplications", async (req, res, next) => {
  try {
    const response = await getLawyerApplications();
    res.status(200).json(response);
  } catch (err) {
    next(err);
  }
...
app.post('/updateLawyerApplicationStatus', async (req, res, next) => {
  ...
  try {
    const response = await updateLawyerApplicationStatus(req.body.applicationId, req.body.status);
    res.status(200).json(response);
  } catch (err) {
    next(err);
  }
